...
namespace Demo
{
  ...
  void SinbadCharacterController::createBody( Ogre::SceneManager *sceneMgr )
  {
    ...
    Ogre::Item *item = sceneMgr->createItem(
                "Sinbad_v2.mesh",
  ...
  void SinbadCharacterController::updateBody( Ogre::Real deltaTime )
  {
    ...
    else if( mCameraController->getCameraMode() == CAM_TPS_MODE )
    {
      ...
      if( mKeyDirection != Ogre::Vector3::ZERO && mBaseAnimID != ANIM_DANCE )
      {
        ...
        if( yawToGoal < 0.0f )
          yawToGoal = std::min<Ogre::Real>( 0.0f, std::max<Ogre::Real>( yawToGoal, yawAtSpeed ) ); //yawToGoal = Math::Clamp<Real>(yawToGoal, yawAtSpeed, 0);
        else if( yawToGoal > 0.0f )
          yawToGoal = std::max<Ogre::Real>( 0.0f, std::min<Ogre::Real>( yawToGoal, yawAtSpeed ) ); //yawToGoal = Math::Clamp<Real>(yawToGoal, 0, yawAtSpeed);
