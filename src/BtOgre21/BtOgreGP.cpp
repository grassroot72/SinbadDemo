...
namespace BtOgre
{
	...
	void VertexIndexToShape::appendV1VertexData( const Ogre::v1::VertexData *vertex_data )
	{
		...
		const auto posElem = vertex_data->vertexDeclaration->findElementBySemantic( Ogre::VES_POSITION );
		...
		const auto vertex = static_cast<unsigned char*>( vbuf->lock( Ogre::v1::HardwareBuffer::HBL_READ_ONLY ) );
		// this pointer will be used a a buffer to write the [float, float, float] array of the vertex
		Ogre::Real* rawVertex{ nullptr };
		...
		for( auto j = size_t{ 0U }; j < vertexCount; ++j )
		{
			...
			mVertexBuffer[previousSize + j] = mTransform * Ogre::Vector3{ rawVertex };
		}
													...
													const Ogre::v1::Mesh::IndexMap *indexMap )
	{
		...
		const auto bneElem = vertex_data->vertexDeclaration->findElementBySemantic( Ogre::VES_BLEND_INDICES );
		assert( bneElem );
	...
	void VertexIndexToShape::appendV1IndexData( Ogre::v1::IndexData *data, const size_t offset )
	{
		...
		if( ibuf->getType() == Ogre::v1::HardwareIndexBuffer::IT_32BIT )
	...
	{
		if( mBounds == Ogre::Vector3(-1, -1, -1) && getVertexCount() )
		{
	...
	btConvexHullShape* VertexIndexToShape::createConvex()
	{
		...
		auto shape = new btConvexHullShape{
				static_cast<btScalar*>( &mVertexBuffer[0].x ), int(getVertexCount()), sizeof(Ogre::Vector3) };
	...
	VertexIndexToShape::~VertexIndexToShape()
	{
		if( mBoneIndex )
		{
			for( auto i = begin( *mBoneIndex ); i != end( *mBoneIndex ); ++i)
				delete i->second;
	...
	StaticMeshToShapeConverter::StaticMeshToShapeConverter( Ogre::v1::Entity *entity, const Ogre::Matrix4 &transform ) :
	...
	StaticMeshToShapeConverter::StaticMeshToShapeConverter( Ogre::v1::Mesh *mesh, const Ogre::Matrix4 &transform ) :
		VertexIndexToShape( transform ),
		mEntity( nullptr ),
		mItem( nullptr ),
		mNode( nullptr )
	...
	void StaticMeshToShapeConverter::addEntity( Ogre::v1::Entity *entity, const Ogre::Matrix4 &transform )
	{
		mEntity = entity;
		mNode = static_cast<Ogre::SceneNode*>( mEntity->getParentNode() );
		mScale = mNode ? mNode->getScale() : Ogre::Vector3::UNIT_SCALE;

		addMesh( mEntity->getMesh().get(), transform );
	}

	void StaticMeshToShapeConverter::addMesh( const Ogre::v1::Mesh *mesh, const Ogre::Matrix4 &transform )
	{
	...
	void VertexIndexToShape::getV2MeshBufferSize( const Ogre::Mesh* mesh, size_t& previousVertexSize, size_t& previousIndexSize )
	{
														   ...
														   const size_t& prevSize )
	{
		...
		case Ogre::VET_HALF4:
			...
			{
				// Stored as 16 bits. Need to use Ogre::Bitwise utilities to extract a floating point form this
				auto pos = reinterpret_cast<const Ogre::uint16*>( requests[0].data );
				requests[0].data += requests[0].vertexBuffer->getBytesPerElement();
				mVertexBuffer[prevSize + i + subMeshOffset] =
						mTransform * Ogre::Vector3{ Ogre::Bitwise::halfToFloat( pos[0] ),
													Ogre::Bitwise::halfToFloat( pos[1] ),
													Ogre::Bitwise::halfToFloat( pos[2] ) };
			}
			break;
		...
		case Ogre::VET_FLOAT3:
			...
			for( size_t i = 0; i < subMeshVerticiesNum; ++i )
			{
				auto pos = reinterpret_cast<const Ogre::Real*>( requests[0].data );
				requests[0].data += requests[0].vertexBuffer->getBytesPerElement();
				mVertexBuffer[prevSize + i + subMeshOffset] = mTransform * Ogre::Vector3( pos );
			}
			break;
	...
	void VertexIndexToShape::requestV2VertexBufferFromVao( Ogre::VertexArrayObject* vao, Ogre::VertexArrayObject::ReadRequestsArray& requests )
	{
	...
	void VertexIndexToShape::extractV2SubMeshIndexBuffer( const size_t& previousSize, const size_t& offset, const bool& indices32, Ogre::IndexBufferPacked* indexBuffer )
	{
		if( indexBuffer )
		{
			...
			if( indices32 )
				loadV2IndexBuffer<Ogre::uint32>( asyncTicket, offset, previousSize, indexBuffer->getNumElements() );
			else
				loadV2IndexBuffer<Ogre::uint16>( asyncTicket, offset, previousSize, indexBuffer->getNumElements() );
		}
	}
	...
	{
		mItem = item;
		mNode = static_cast<Ogre::SceneNode*>( mItem->getParentNode() );
		mScale = mNode ? mNode->getScale() : Ogre::Vector3::UNIT_SCALE;
	...
	void StaticMeshToShapeConverter::addMesh( const Ogre::Mesh* mesh, const Ogre::Matrix4& transform )
	{
		...
		for( const auto& subMesh : mesh->getSubMeshes() )
		{
			...
			Ogre::VertexArrayObject::ReadRequestsArray requests;
			 // /!\ Don't forget that this call will map async tickets in the request
			requestV2VertexBufferFromVao( vao, requests );
			...
			extractV2SubMeshIndexBuffer( prevIndexSize + appendedIndexes,
										 indexOffset,
										 vao->getIndexBuffer()->getIndexType() == Ogre::IndexBufferPacked::IT_32BIT,
										 indexBuffer );
	...
	AnimatedMeshToShapeConverter::AnimatedMeshToShapeConverter( Ogre::v1::Entity *entity, const Ogre::Matrix4 &transform ) :
		VertexIndexToShape( transform ),
		mEntity( nullptr ),
		mNode( nullptr ),
	...
	void AnimatedMeshToShapeConverter::addEntity( Ogre::v1::Entity *entity, const Ogre::Matrix4 &transform )
	{
		// Each entity added need to reset size and radius
	...
	void AnimatedMeshToShapeConverter::addMesh( const Ogre::v1::MeshPtr &mesh, const Ogre::Matrix4 &transform )
	{
		// Each entity added need to reset size and radius
		// next time getRadius and getSize are asked, they're computed.
		mBounds = Ogre::Vector3( -1, -1, -1 );
													   ...
													   Ogre::Vector3 &box_kCenter )
	{
		...
		box_afExtent.x = Ogre::Real(0.5f)*( fY0Max - fY0Min );
		box_afExtent.y = Ogre::Real(0.5f)*( fY1Max - fY1Min );
		box_afExtent.z = Ogre::Real(0.5f)*( fY2Max - fY2Min );

		box_kCenter +=
			 0.5f*( fY0Max + fY0Min )*box_akAxis[0] +
