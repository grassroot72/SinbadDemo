...
namespace Demo
{
                                                 ...
                                                 Ogre::Real deltaZoom )
  {
    ...
    // bound the pitch
    if( !( mPivotPitch + deltaPitch > 25.0f && deltaPitch > 0.0f ) &&
        !( mPivotPitch + deltaPitch < -25.0f && deltaPitch < 0.0f ) )
    {
    ...
    // bound the zoom
    if( !( dist + distChange < 0.1f && distChange < 0.0f ) &&
  ...
  void SinbadCameraController::mouseMoved( const SDL_Event &evt )
  {
    ...
    else if( mCameraMode == CAM_TPS_MODE )
    {
      if( evt.wheel.y == 1 || evt.wheel.y == -1 )
      {
        ...
        updateCameraGoal( 0.0f, 0.0f, -0.1f*evt.wheel.y );
      }
      else
      {
